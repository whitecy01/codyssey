...
#### 1.3.1 인가 코드
클라이언트와 리소스 소유자 사이에 권한 부여 서버를 중개자로 사용하여 권한 부여 코드를 얻습니다. 클라이언트는 리소스 소유자에게 직접 권한을 요청하는 대신, 리소스 소유자를 권한 부여 서버로 연결합니다(RFC2616에 정의된 사용자 에이전트를 통해). 그러면 권한 부여 서버는 리소스 소유자를 다시 클라이언트로 연결해 줍니다. 권한 부여 서버는 리소스 소유자를 클라이언트로 연결하기 전에 리소스 소유자를 인증하고 권한을 획득합니다. 리소스 소유자는 권한 부여 서버에서만 인증을 받기 때문에 리소스 소유자의 자격 증명은 클라이언트와 공유되지 않습니다.